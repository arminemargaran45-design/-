...
WORK="/tmp/sws-desktop-v10-$STAMP"
SITE="$WORK/sws-site"
RELEASE="/opt/styling-wrap-studio/releases/$STAMP-desktop-v10"
...
grep -q 'DESKTOP V10' "$SITE/styles.css" || fail "desktop V10 CSS missing"
...
printf '%s' "$CSS" | grep -q 'DESKTOP V10' || fail "desktop V10 not served"
printf '%s' "$CSS" | grep -q "height:1050px!important" || fail "wide desktop story height fix missing"
printf '%s' "$CSS" | grep -q "background-image:url('/assets/sws/mercedes-front.jpg')!important" || fail "desktop story local image fallback missing"
printf '%s' "$CSS" | grep -q ".reveal," || fail "desktop reveal fallback missing"
printf '%s' "$CSS" | grep -q '@media(min-width:981px)' || fail "desktop-only media rule missing"
printf '%s' "$CSS" | grep -q '@media(max-width:980px)' || fail "mobile CSS unexpectedly missing"
...
printf '\n====================================================\n'
printf ' SWS DESKTOP V10 ACTIVE\n'
printf ' Premium graphite/ice-silver/steel-blue palette: ACTIVE\n'
printf ' Desktop composition: FIXED\n'
printf ' Desktop black dead zones: FIXED\n'
printf ' Mobile rules preserved: YES\n'
